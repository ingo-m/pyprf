...
def funcLnTrRm(idxPrc, aryFuncChnk, varSdSmthSpt, queOut):
    ...
    ----------
    idxPrc : int
        Process ID (for sorting return values in case of multi-threading).
    aryFuncChnk : np.array
        Data to perform linear trend removal on, shape: aryFuncChnk[time,
        voxels].
    varSdSmthSpt : float
        Dummy variable, only included for consistency with other functions
        using the same parallelisation.
    queOut : multiprocessing.queue
        Multiprocessing queue to place results on. If `None`, processed
        data is returned directly.

    Returns
    -------
    None or aryFuncChnk ( : np.array)
        If a multiprocessing queue is provided as input, results are put
        on the queue (a list containing the process ID `idxPrc`, and the
        preprocessed data, `aryFuncChnk`). Otherwise `aryFuncChnk` is
        returned directly.
    ...
    # If no queue was provided, return preprocessed data:
    if queOut is None:
...
def funcSmthTmp(idxPrc, aryFuncChnk, varSdSmthTmp, queOut):
    ...
    idxPrc : int
        Process ID (for sorting return values in case of multi-threading).
    aryFuncChnk : np.array
        Data to perform linear trend removal on, shape: aryFuncChnk[timepoints,
        voxels].
    varSdSmthTmp : float
        Extent of temporal smoothing (SD in units of array index).
    queOut : multiprocessing.queue
        Multiprocessing queue to place results on. If `None`, processed
        data is returned directly.
