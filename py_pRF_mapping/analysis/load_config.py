...
strCsvCnfg = '/home/john/Desktop/test/config.csv'
...
with open(strCsvCnfg, 'r') as fleConfig:
    ...
    for lstTmp in csvIn:
        ...
        # Skip comments (i.e. lines starting with '#') and empty lines. Note:
        # Indexing the list (i.e. lstTmp[0][0]) does not work for empty lines.
        # However, if the first condition is no fullfilled (i.e. line is empty
        # and 'if lstTmp' evaluates to false), the second logical test (after
        # the 'and') is not actually carried out.
...
if lgcPrint:
    print('---Number of pRF sizes to model: ' + str(dicCnfg['varNumPrfSizes']))
...
# Minimum pRF model size (standard deviation of 2D Gaussian) [degrees of visual
# angle]:
dicCnfg['varPrfStdMin'] = float(dicCnfg['varPrfStdMin'])
if lgcPrint:
    print('---Minimum pRF model size: ' + str(dicCnfg['varPrfStdMin']))

# Maximum pRF model size (standard deviation of 2D Gaussian) [degrees of visual
# angle]:
...
if lgcPrint:
    print('---Create pRF time course models: ' + str(dicCnfg['lgcCrteMdl']))
...
# experiment.
strPathPng = '/media/john/DATADRIVE1/MRI_Data_PhD/04_ParCon/20161212_02/nii_distcor/retinotopy/pRF_stimuli/frame'
...
# extension.
strPathMdl = '/media/john/DATADRIVE1/MRI_Data_PhD/04_ParCon/20161212_02/nii_distcor/retinotopy/pRF_results/pRF_model_tc'
